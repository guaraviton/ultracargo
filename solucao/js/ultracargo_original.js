...
function UltraCargo()
{
    ...
    this.doVerifySession = true; // Se for false, não verifica se a sessão expirou (usuário não logado)
	this.init();
}
...
//	try {
		...
		else{ //Retorna o valor armazenado em uma session
			this.oXHttp.send();
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[SessionText]");
//	}
//	finally {
	obj_Form = null;
    	obj_Field = null;
...
//	try {
		...
		if (opener != null) {
			...
			//Verifica se existe um diretório virtual.
		...
		*/
		...
		{
			this.init.oXD = new ActiveXObject("MSXML2.FreeThreadedDOMDocument");
		}
		this.init.oXD.loadXML("<root/>");
		if (this.init.oXDt == undefined) {
			this.init.oXDt = new ActiveXObject("MSXML2.FreeThreadedDOMDocument");
		}
		this.init.oXDt.loadXML("<root/>");
		if (this.init.oXGr == undefined) {
			this.init.oXGr = new ActiveXObject("MSXML2.FreeThreadedDOMDocument");
		}
		...
		{
			this.init.oXCheck = new ActiveXObject("MSXML2.FreeThreadedDOMDocument");
		}
		this.init.oXCheck.loadXML("<root/>");
		//****************************************************************
		//XML  para guardar itens selecionados de uma grid, porém,  que
		//não serão guardados em outra grid.
		//****************************************************************
		if (this.init.oXCheckXML == undefined)
		{
			this.init.oXCheckXML = new ActiveXObject("MSXML2.FreeThreadedDOMDocument");
		}
		this.init.oXCheckXML.loadXML("<root/>");
        //Atualiza com as variáveis de sessão
...
//	try {
		...
		var obj_Form  = document.getElementById(this.setopenPopUp.sForm); //Formulário do preenchimento
		var obj_Field = this.setopenPopUp.oDsField;		           //Objeto>> this qd do onBlur e null qd do onClick
		...
		if(vwAtivaPopUp == true){
			...
			//Recupera o id para popular pop-up
			//obj_Form.all(arr_FormField[1]).value = str_Key;
...
//*********************************************
// Rotina para ordenar o XML de Grid(local)
//*********************************************
UltraCargo.prototype.Fu_OrdenaXML = function(paPathPai,paPathLine,paOrdenarCampo)
{
	...
	//******************************************
	// Carrega XML de Ordem a partir do XML
	// que deverá ser ordenado. 
	//******************************************
	vwObjXMLOrder.loadXML(this.init.oXGr.xml);
	//******************************************
	// Seleciona os node do Ordenador 
	// a partir do path informado
	//******************************************
	...
	if(vwNodesOrder!= null)
	{
		...
		//**********************************
		//Remove o ultimo caracter de split
		//**********************************
		vwStringCampos   =  vwStringCampos.substr(0,vwStringCampos.length-1);  
	}
	//******************************************
	// Após montar a string com os campos que 
	// serão ordenas, joga para dentro de um 
	// objeto array para efetuar o sort.
	//******************************************
	var vwArrOrdenar  = new Array() 
	vwArrOrdenar = vwStringCampos.split('|')
	vwArrOrdenar.sort();
	//******************************************
	// Remove os Nodes para serem inseridos, 
	// novamente, mas pela ordem definida
	// no array.
	//******************************************
	...
	//******************************************
	// Após Remove os Nodes do XML,
	// começa a inserir novamente, porém
	// na ordem defina pelo array
	//******************************************
...
UltraCargo.prototype.Fu_RetLinhaAtualSele = function(psTableId,paXML)
{
	//*****************************************************************
	// Programador: SOTTO
	// Funcionalidade: Retorna a ultima linha selecionada, a partir
	// de um XML informado
	//*****************************************************************
	var vwLinhaSele=0;
	var vwCheck = '' 
	...
	// A partir da linha selecionada, procura pelo XML de dados
...
UltraCargo.prototype.AtualizaColsToValidade = function(psTableId,paNewColToValidade)
{
	...
	{
		var vwValidate  = this.cTF.oNds[iN].getAttribute('validate');  //aqui
...
UltraCargo.prototype.CriaCheckXML = function(paNomeForm,paLinhaXML,paNomeFormEstru)
{
	...
	//**********************************************************************************
	//Antes de atualizar, verifica se registro já está no XML de CHECK, através
	//das chaves 
	//**********************************************************************************
	...
	{
		//Verifica se o Nó, é válido, ou seja, traz os atributos
		var vwPos  = 0;
		var vwValor  =  vwObjNodes[vwi].xml; 
		vwPos  = vwValor.indexOf('page',0)
		if( vwPos != -1) 
		{
			// Na linha que vai ser gravada, concatena os valores para
			vwConc_1 = ''
			vwConc_2 = ''
...
//******************************************
// Rotina para adicional linhas em 
// outro XML a partir de um XML infromado.
//******************************************
...
{
	//*********************************************
	//Retorna os campos da chave, para procurar a
	//linha no XML da grid de dados.
	//*********************************************
	var vwChave = ''; 
	...
	//*******************************************************
	//Depois que exclui, atualiza as linhas do line atributo
	//*******************************************************
	this.cTF.oNds = this.init.oXCheck.selectNodes("root/*/row");
	for (var iN = 0; iN < this.cTF.oNds.length; iN++) 
	{
	...
	if(vwNodes != null && this.cTF.oNd != null )
	{
		for (var vwi = 0; vwi <= vwNodes.length-1; vwi++) 
		{
			...
			if(vwConc_1 == paChaveConc)
			{
				...
				var vwRota = this.init.oXGr.selectSingleNode(this.sXPath); //aqui
				if(vwRota != null)
				{
...
UltraCargo.prototype.CriaXMLtoCheck = function(paTipo,paNomeForm,paLinhaXML)
{
	...
	if(paTipo == 1 || paTipo ==null)   //Inicia o XML
	{
...
{
	//****************************************************
	// Programador:Marco Sotto - 09/05/2005
	// Finalidade:  Excluir nós do XML que
	// guarda itens selecionados de uma 
	// grid.
	//-------------------------------------------
	//Explicação:  No parâmetro psTableId, 
	//deve ser informado o nome do Nó(table) que deverá
	//ser excluído. Caso nenhum seja informado,  
	//todos os rows de cada table serão excluídos  
	//****************************************************
...
UltraCargo.prototype.ClearXmlData_GLocal = function(psTableId)
{
	this.cTF.oNds = this.init.oXDt.selectNodes("root/" + psTableId);
	
	for (var icPTF = 0; icPTF <= this.cTF.oNds.length-1; icPTF++){
		
		this.sPg = this.cTF.oNds[icPTF].getAttribute("page");
		this.oSNds = this.cTF.oNds[icPTF].selectNodes("row");
		
		for (var iSNds = 0; iSNds <= this.oSNds.length-1; iSNds++) 
		{
			this.oSNd = this.init.oXDt.selectSingleNode("root/" + psTableId + "[@page=" + this.sPg + "]");
			if (this.oSNd != null)
			{
				this.oSNd.removeChild(this.oSNds[iSNds]);
			}
		}
...
{
	//********************************************************************
	//Definição da Função: Realiza limpeza nos nó de dados da 
	//tabela informada
	//********************************************************************
	this.cTF.oNds = this.init.oXDt.selectNodes("root/" + psTableId + "/*");
	for (var iN = 0; iN <= this.cTF.oNds.length-1; iN++) 
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[Xml]");
//	}
}
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[getXmlData]");
//	}
}
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[getXmlStruct]");
//	}
}
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[multiForm]");
//	}
}
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[keyValues]");
//	}
}
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[mensagem]");
//	}
}
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[code]");
//	}
}
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[groupData]");
//	}
}
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[processPage]");
//	}
}
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[showItensComboIsEmpty]");
//	}
}
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[reset]");
//	}
}
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[check]");
//	}
//	finally {
//	}
}
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[popUpField]");
//	}
}
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[popUpGrid]");
//	}
}
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[popUpFill]");
//	}
}
...
UltraCargo.prototype.setSession = function() 
{
	...
	if (loXML.selectSingleNode("root").getAttribute("value") != "OK"){
		alert("[getXmlData]:" + loXHttp.statusText)
	}
					...
					if (this.oElmnt != null) 
					{
						...
						switch(this.sTgNm) {
							...
							case "input-text":
								...
								{
									case "dec":
										//Pega os Parâmentros necessários
...
//	try {
		...
		//******************************************************************************
		//Propriedades criadas por Marco Sotto
		/////////////////////////DEFINIÇÃO//////////////////////////////////////////////
		//1-BotaoEspecial: 
		//		Recebe o valor true ou false:
		//		true:
		//		Consiste outras propriedades 
		//		relacionadas(ImgBotaoEspecial,MainCaseBotaoEspecial)
		//		false:
		//		Não é necessário consistir outras 
		//		propriedades relacionadas.
		//2-ImgBotaoEspecial
		//		Nome da imagem que deverá ser 
		//		mostrada como botão. Passar somente
		//		o nome da imagem(Ex.: botao_confirmar.gif). O caminho deverá ser o mesmo
		//		já utilizados por botões normais(Editar,Excluit etc.) da grid.
		//3-MainCaseBotaoEspecial
		//		Este deve ser o nome do case que o evento click do botão especial deverá
		//		chamar(Ex.: frm_pesquisa_botaoespecial). O mesmo poderá receber qualquer 
		//		valor, mas por gentileza seja breve na definição do mesmo, evitando
		//		nomes extensos.
		//4-ToolTipBotaoEspecial
		//		Descritivo que deverá aparecer quando o usuário passar com o mouse
		//		sobre a imagem do Botão Especial.
...
UltraCargo.prototype.addTable = function() 
{
		if (this.validateTable()) 
		{
			...
			//*Data: 14/04/2005
			//*Alteração: Caso o tipo da grid seja igual a CHECK, 
			//*cria, automaticamente, o xml que deverá guardar
			...
			//*itens selecionados de uma grid, porém sem a neces-
...
UltraCargo.prototype.validateTable = function() 
{
	...
	try {
        ...
        // Controle de acesso. Inibe botões de edição do grid se usuário não possuir privilégios para edição. By Jackson
        ...
        if(this.grid.BotaoEspecial ==true)
        {
			...
			{
				this.CompMensagem  += "Imagen não informada para o Botão Especial.\n" 
		...
		if (this.iFrom != -1 && this.iTo != -1) 
		{
			...
			if (this.oTblFrom != null && this.oTblTo != null) 
			{
				...
				if (this.oTable == null) 
				{
					...
					// linha inserida em 09/02/2005 para controle de pages
				...
				if (this.oTable == null) 
				{
					...
					this.oNd.setAttribute("tipo","table");
					this.oNd.setAttribute("pk",this.sPKTo);
					// linha inserida em 10/02/2005 para controle de pages 
					this.oNd.setAttribute("page",1);
					this.oRoot.appendChild(this.oNd);
					this.oTable = this.init.oXDt.selectSingleNode("root/" + psTableTo + "[@pk=\"" + this.sPKTo + "\"]");
				}
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[checkForChange]");
//		return false;
//	}
//	finally {
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[clearMemory]");
//	}
//	finally {
		this.cTF.oNds = null;
		this.cTF.oNd = null;
...
//	try {
		...
		//****************************************************
		//Altera os atributos da paginação.
		//****************************************************
					...
					if (this.oNd != null) 
					{
						...
						switch(this.sOper) 
						{
							...
							case "check_del":
								...
								break;
							case "ins":
								if (this.verifyGrid() && this.verifyGridCons())  //VOLTAR AQUI 
								//if (this.verifyGridCons())
								{
									this.insertRow();
							...
							case "edt":
								//********************************************************************
								//Nesta operação,deve ser definido qual tipo de botão será mostrado
								//********************************************************************
							...
							case "nxt":
								...
								//********************************************
								//Processo criado para controlar paginação
								//de grid's do tipo check
								//********************************************
						...
						if (!this.bCancelOperation) 
						{
							for (var iF = 0; iF < document.getElementsByTagName("IFRAME").length; iF++) {
								if (document.getElementsByTagName("IFRAME")[iF].getAttribute("id") == this.sFrmId) 
								{
									...
									self.frames[iF].cancelGrid(); //aqui 
									break;
								}
							}
						}
						...
						if (this.bExecuteOnClientServer)
						{
							...
							if (this.oElmnt != null)
							{
								...
								//*******************************************************
								//Processo idealizado para tratamento de grid de multipla
								//seleção.
								//*******************************************************
								if(this.sOper_old == 'chk')
								{
									//Procura pelo tipo de grid
								...
								if(vwOper  == 'chk')
								{
									...
									if(this.TipoGrid == 'CHECK_XML')
									{
										if(this.XMLCheck != '' && this.XMLCheck != null)
										{
											this.sXML = obj_Ultra.getSelectedItens(this.sFrmId,true);
											//******************************************************
											// Garante que somente a linha selecionada será enviada  
											// para criação do XML
											//******************************************************
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[dataGrid]");
//	}
//	finally {
		this.sOper = null;
...
//	try {
		...
		if (this.oDt != null) 
		{
			...
			if (this.oNd != null) 
			{
				...
				if (this.bValidate && this.sPK != "") {
					...
					this.oRw = this.init.oXGr.selectSingleNode("root/" + this.sFrmId + "[@pk=\"" + this.sPKElement + "\"]/row[" + this.sXPath + "]");
					if (this.oRw == null) {
						...
						this.oRw = this.init.oXDt.selectSingleNode("root/" + this.sFrmId + "[@pk=\"" + this.sPKElement + "\"]/row[" + ((this.bIB != "true") ? "@status!='ins' and " : "") + this.sXPath + "]");
					}
										
					if (this.oRw != null && this.duplicar == "false" && this.sOper == "ins") 
					{
		...
		if (this.sMsgBdy != "") 
		{
			//alert(this.sMsgHder + this.sMsgBdy);
			msgBox(" - Os seguintes campos obrigatórios estão vazios:\n" + this.sMsgBdy);
		}
		return this.bValidate;
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[verifyGrid]");
//		return false;
//	}
//	finally {
		this.sMsgHder = null;
...
//***********************************************
// Verifica consistências especiais para colunas
//***********************************************
UltraCargo.prototype.verifyGridCons = function() 
{
	//*******************************************
	//Esta função tem a finalidade de realizar
	//operaçãoes entre duas colunas para cada 
	//conjunto.
	//Formado esperado: [col1|col2|operador]
	//Exemplo: [1|2|<]
	//*******************************************
	var vwConjunto		=	"";
	var vwConjunto_Str =	"";
			...
			if (this.oNds[0] != null) 
			{
				...
				for(var vwi = 0;vwi<=vwConjunto.length-1;vwi++)
				{
					...
					if (this.oNd != null && this.oNd_col2 != null ) 
					{
						...
						//******************************************************************************
						//Verifica se as colunas a serem comparadas possuem a mesma tipagem
						//******************************************************************************
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[insertRow]");
//	}
//	finally {
		this.oDt = null;
		this.sLnk = null;
...
//	try {
		...
		if (this.oDt != null) {
			...
			if (this.oNd != null) {
				...
				//this.bFK = (this.oNd.getAttribute("enable_fk") != null) ? this.oNd.getAttribute("enable_fk") : "false";
			}
			this.sXPath += "[@pk=\"" + this.sPKElement + "\" and @page='" + this.sPg + "']";
			if (this.iLn != "") {
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[editRow]");
//	}
//	finally {
		this.oDt = null;
		this.iLn = null;
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[updateRow]");
//	}
//	finally {
		this.oDt = null;
		this.iLn = null;
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[deleteRow]");
//	}
//	finally {
		this.oDt = null;
		this.iLn = null;
				...
				if (this.oNd.getAttribute("move_action") != "true") 
				{
					...
					else 
					{
						if (this.sChk == "true") 
						{
							...
							{
								this.oNd = this.init.oXGr.selectSingleNode(this.sXPath + "/row[@line='" + this.iLn + "']"); //aquides
								if (this.oNd != null) 
								{
									this.oNd = this.oNd.cloneNode(true);
									this.oNd.setAttribute("status","chk_ins");
						...
						else 
						{
							...
							if (this.oNd != null) 
							{
								...
								if(vwDelOnDesChk_pop == "true")
								{
									...
									//***************************************************
									//Cria um novo nó no GR, para poder inserir novamente
									//(referencia)
									//***************************************************
							...
							{
								//************************************************************
								//Verifica se o atributo para excluir está ativo para a grid
								//************************************************************
								var vwPath =  "root/" + this.sFrmId;
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[selectRow]");
//	}
//	finally {
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[linkRow]");
//	}
//	finally {
		this.oDt = null;
		this.iLn = null;
...
//	catch(oException) {
//		this.exception(oException,"UltraCargo[changePage]");
//	}
//	finally {
		this.oDt = null;
		this.iPage = null;
					...
					if (this.oDt != null) {
						...
						if (this.sPK != "" && this.sDtTp == "table" && this.sDtExec == "local") {
							...
							if (this.oDt != null) {
								...
								if (this.oDts[0] != null) {
								    qtd_Nodes = this.oDts.length;
									for (var iD = 0; iD < qtd_Nodes; iD++) {
										this.sDtTp = (this.oDts[0].getAttribute("status") != null) ? this.oDts[0].getAttribute("status") : "";
										switch(this.sDtTp) {
											case "srv_upd":
											case "srv_del":
												
												this.oDt.removeChild(this.oDts[0]);
												
												break;
											case "ins":
												this.oDts[0].setAttribute("status","server");
												this.oRoot = this.init.oXGr.selectSingleNode("root/" + this.sNdId + "[@pk=\"" + this.sPK + "\"]");
												if (this.oRoot == null) {
													this.init.oXGr.documentElement.appendChild(this.oDts[0].parentNode.cloneNode(false));
													this.oRoot = this.init.oXGr.selectSingleNode("root/" + this.sNdId + "[@pk=\"" + this.sPK + "\"]");
												}
												this.oRoot.appendChild(this.oDts[0].cloneNode(true));
												this.oDt.removeChild(this.oDts[0]);
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[updateGrid]");
//	}
//	finally {
		this.oXD = null;
		this.oNds = null;
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[updateFormData]");
//	}
//	finally {
		this.asForm = null;
		this.oXD = null;
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[setDataLink]");
//	}
//	finally {
		this.oXD = null;
		this.oNd = null;
...
UltraCargo.prototype.generateTableContent = function(psTableId) 
{
		...
		var vwChave  = ''
		var vwobjXML_NOTIN   = new ActiveXObject("MSXML2.FreeThreadedDOMDocument");
		
		this.oXPg = (arguments[1] != null) ? arguments[1] : "";
		...
		if (this.oNd != null) 
		{
			...
			//********************************************************************************
			//Caso seja um pop de check, deverá pegar a paginação da grid de seleção
			//********************************************************************************
				...
				for (var iX = 0; iX < 2; iX++) 
				{
					...
					switch(iX)
					{
						case 0:
							...
							{
								this.oPNds = this.init.oXGr.selectSingleNode("root/" + psTableId + "[@pk=\"" + this.sPKElement + "\" and @page='" + this.sPg + "']");
					...
					if (iX == 0 && this.oXPg != ""){
						...
						{	
							this.bExecuteOnClientServer = false;
							this.oNXD = this.init.oXD.selectSingleNode("root/" + this.sFrmId);
							if (this.oNXD != null) 
							{
								if (this.oNXD.getAttribute("execute") != null) 
								{
									if (this.oNXD.getAttribute("execute") == "server") 
									{
										this.bExecuteOnClientServer = true;
									}
								}
							...
							if (this.bExecuteOnClientServer == false)
							{
							
								for (var iR = 0; iR < this.oNds.length; iR++) 
								{
									this.oNXDt = this.init.oXDt.selectNodes("root/" + psTableId + "[@page='" + this.sPg + "']/row[@status='srv_del' or @status='srv_upd']");
									
									this.iLineXGr = (this.oNds[iR].getAttribute("line") != null) ? this.oNds[iR].getAttribute("line") : "";
									this.iPageXGr = (this.oNds[iR].getAttribute("page") != null) ? this.oNds[iR].getAttribute("page") : "";
									
									for (var iXDt = 0; iXDt < this.oNXDt.length; iXDt++)
									{	
										this.iLineDt = (this.oNXDt[iXDt].getAttribute("line") != null) ? this.oNXDt[iXDt].getAttribute("line") : "";
										this.iPageDt = (this.oNXDt[iXDt].getAttribute("page") != null) ? this.oNXDt[iXDt].getAttribute("page") : "";
										
										if (this.iLineXGr == this.iLineDt && this.iPageXGr == this.iPageDt)
										{
											this.oPNds.removeChild(this.oNds[iR]);
											
											if (this.oNXDt[iXDt].getAttribute("status") == "srv_del")
											{
												this.oNXDt[iXDt].setAttribute("status","del");
											}
											else if (this.oNXDt[iXDt].getAttribute("status") == "srv_upd")
											{
												this.oNXDt[iXDt].setAttribute("status","upd");
											}
											
											this.oPNds.appendChild(this.oNXDt[iXDt].cloneNode(true));
											
											if (this.oNXDt[iXDt].getAttribute("status") == "del")
											{
												this.oNXDt[iXDt].setAttribute("status","srv_del");
											}
											else if (this.oNXDt[iXDt].getAttribute("status") == "upd")
											{
												this.oNXDt[iXDt].setAttribute("status","srv_upd");
											}
											
											//O append coloca o nó no fim da lista. O replace recoloca no nó no lugar certo
											if (iR != this.oNds.length - 1)
											{
												this.oNN = this.oPNds.removeChild(this.oPNds.selectNodes("row").item(this.oNds.length - 1));
												this.oON = this.oPNds.replaceChild(this.oNN, this.oPNds.selectNodes("row").item(iR));
												this.oPNds.appendChild(this.oON);
											}
						...
						for (var iR = 0; iR < this.oNds.length; iR++) 
						{
								...
								switch(this.tipogrid) 
								{
									case "CHECK_SELE":
										...
										if(vwXML_NOTIN != '' && vwXML_NOTIN != null)
										{
											...
											{
												this.sHDscr += "<td nowrap  align='center' valign='middle' class='" + ((this.sStts == "del") ? this.asBGC[2] : this.asBGC[(this.iBGColor%2)]) + "'>" //aquiver
										...
										{
											this.sHDscr += "<td nowrap  align='center' valign='middle' class='" + ((this.sStts == "del") ? this.asBGC[2] : this.asBGC[(this.iBGColor%2)]) + "'>" //aquiver
												+ "<input id='Chk' type='checkbox' line='" + this.iLn + "'"  + " page='" + this.sPg + "' style='cursor:hand'" + ((this.sStts == "del") ? " disabled" : "") + " onclick='setCheck(this);' onkeypress='setCheck(this);' mark='false'>" //aquiver
												+ "</td>";
										} 	
										break;
									default:
							...
							if (this.bEdtBt == "true" && this.bEdtDet == "false") 
							{
								...
								else
								{
									...
									{
										// Adicionado este if para não colocar espaços no edit com multipla seleção
										if (this.bSelBt != "multiple") 
										{
							...
							if (this.bDelBt == "true")
							{
								...
								else 
								{
									...
									/*
									  ...
									  @Descrição:	Esta alteração foi necessária para a montagem 
													correta da grid juntamente com os botões de alteração , inclusão e exclusão.
													O ocorria devido a montagem de uma  <td> com alguns icones transparentes que serviam
													para alinhar o cabeçalho com os valores buscados do banco de dados.
									  @Solução :    Comentário de linhas excessivas no código
									...
									*/
									...
									//******************************************************************
									//Caso a grid seja de pop-up de multipla seleção, chama outro método
									//******************************************************************
									switch(this.tipogrid) 
									{
										case "CHECK":
							...
							//************************************************************************************
							// Montagem da linha para o botão 
							//************************************************************************************
							if (this.BotaoEspecial == "true")
							{	
								this.sHDscr += "<td nowrap align='center' valign='middle' class='" + ((this.sStts == "del") ? this.asBGC[2] : this.asBGC[(this.iBGColor%2)]) + "'>";
							...
							if (this.oElmnts[0] != null) 
							{
								...
								for (var iC = 0; iC < this.oElmnts.length; iC++) {
									...
									if (this.oNd != null) 
									{
										this.oElmnt = this.oNds[iR].selectSingleNode(this.oNd.nodeName);
										if (this.oElmnt != null) {
											this.sTipo = "";
											this.sPK = (this.oElmnt.getAttribute("pk") != null) ? this.oElmnt.getAttribute("pk") : "";
											if (this.sHidFlds != "") 
											...
											if (this.sPK == "false") 
											{
												...
												switch(this.sTipo) 
												{
													...
													default:
														...
														if(this.sTipo == 'ins') // Prepara o valor para ser mostrado sem decimal  
														{
														...
														{
															this.sHCnt += "<td class='" + ((this.sStts == "del") ? this.asBGC[2] : this.asBGC[(this.iBGColor%2)] + "' onclick='linkedGrid(this)' style='cursor:hand;text-decoration:underline") + "' line='" + this.iLn + "' align='" + this.sAlign + "' nowrap>&nbsp;" + this.sVl + "&nbsp;</td>";
															if (this.iPos < (this.iFixed + 1)) 
															{
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[getSequence]");
//		return 10000;
//	}
//	finally {
		this.iSequence = null;
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[combo]");
//	}
}
...
UltraCargo.prototype.validateCombo = function() 
{
	...
	try {
		...
		this.exception(oException,"[validateCombo]");
		return false;
	}
	finally {
		this.bValidate = null;
...
UltraCargo.prototype.fillCombo = function() 
{
		...
		if (this.validateCombo()) 
		{
			...
			//***************************************************************************
			//Rotina para trazer selecionado o combo quando existir somente um item
			//Conforme solicitado, esta alteração só é feita para combos que estão 
			//em formulário 
			//***************************************************************************
			if (!this.combo.grid && this.combo.autosele == true)  // Combo de Forms 
			{
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[fillCombo]");
//	}
//	finally {
		this.sResp = null;
		this.oFrmFrom = null;
				...
				if (this.sFldsFrom != "" && this.sFldsTo != "") {
					...
					if (this.oNd != null) {
						...
						for (var iT = 0; iT < this.oNds.length; iT++) {
							...
							if ((this.sSelectedItens == "" && this.init.bFillAllItens) || (this.sSelectedItens == this.sVl.split("|")[0])) {
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[linkCombo]");
//	}
//	finally {
		this.init.bFillAllItens = false;
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[terminateCombo]");
//	}
}
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[formData]");
//	}
//	finally {
		this.oXD = null;
		this.oNd = null;
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[loadData]");
//	}
//	finally {
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[updateData]");
//	}
//	finally {
		this.sSpecificFormId = null;
		this.oFrm = null;
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[clearForms]");
//	}
//	finally {
		this.bUnique = null;
		this.asForm = null;
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[clearLinkedForm]");
//	}
//	finally {
		this.bLoop = null;
		this.oFrm = null;
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[clearLinkedFrames]");
//	}
//	finally {
//	}
}
...
UltraCargo.prototype.send = function() 
{
		...
		// Muda o foco para janela para funcionar a associação dos eventos nos campos str
		//window.focus();
...
UltraCargo.prototype.postXML = function() 
{
		...
		//Se não for bem sucedido interrompe a execução(para exibir somente o botão cancelar)
		//if (this.oXHttp.status != 200){
		//	msgBox("Erro: " + this.oXHttp.statusText + " na execução do caminho " + this.init.processPage);
		//	this.serro = false;
		//}
		...
		if (this.oXD.parseError == 0)
		{
			...
			if (this.oNd == null)
			{
				...
				for (var iM = 0; iM < this.asForm.length; iM++)
				{
					...
					if (this.oNd != null)
					{
						this.sResp = "<root>" + this.oNd.xml + "</root>";
					}
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[send : postXML]");
//		return false;
//	}
//	finally {
//	}
...
UltraCargo.prototype.validateToSend = function() 
{
	...
	try {
		...
		/*if (!this.bValidate) 
		{
			...
			if(vwTipoMensagem == 1)  // Login e sessão 
			{
		...
		this.exception(oException,"[validateToSend]");
		return false;
	}
	finally {
		this.bValidate = null;
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[terminateToSend]");
//	}
//	finally {
//	}
}
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[verifyFields]");
//		return false;
//	}
//	finally {
		this.sXML = null;
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[validateDataGrid]");
//		return "";
//	}
//	finally {
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[setKeyValues]");
//		return false;
//	}
//	finally {
		this.sKV.bAll = null;
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[pageLoader]");
//	}
//	finally {
		this.oXD = null;
		this.oFrm = null;
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[getXml]");
//	}
//	finally {
			...
			if (this.bBrf) 
			{
				...
				for (var iN = 0; iN < this.oNds.length; iN++) {
					...
					if (("," + this.sFrmOper + ",").indexOf("," + psOperation + ",") != -1) {
						...
						for (var iN = 0; iN < this.oFraNds.length; iN++) {
							this.oFra = this.oFraNds[iN];
										...
										if (this.oRws[0] != null) 
										{
											for (var iR = 0; iR < this.oRws.length; iR++) {
												...
												if (this.oRw != null) 
												{
													if(this.sDupl != 'true')
													{
														this.oRws[iR].parentNode.removeChild(this.oRws[iR]); //aquivol
													}	
												}
											}
										}
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[popUp]");
//	}
//	finally {
		this.sUrl = null;
...
//	try {
		...
		if (this.oXD.parseError == 0) {
			...
			if (this.oNd != null) {
				...
				if (this.oNd != null) 
				{
					//Para evitar problemas, com dados alterados, está função sempre retornará true
					return true;
				}
			}
		}
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[notExistDataLink]");
//		return true;
//	}
//	finally {
		this.oXD = null;
...
//	try {
		this.oXDom = new ActiveXObject("MSXML2.FreeThreadedDOMDocument")
		this.oXDom.loadXML(psXML);
		if (this.oXDom.parseError == 0) {
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[saveXmlQuery]");
//	}
//	finally {
		this.oXDom = null;
		this.oNd = null;
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[rebuild]");
//	}
//	finally {
		this.sCase = null;
		this.iPos = null;
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[getSelectedItens]");
//	}
//	finally {
		this.gSI.bResp = null;
		this.gSI.oFras = null;
...
UltraCargo.prototype.showMessage = function(psXML) 
{
		...
		if (this.oXD.parseError == 0) {
			...
			if (this.oNds[0] != null) {
				if (this.shwMsg == true) {
					...
					if ((this.oNds[0].getAttribute("value") >= 1) && (this.oNds[0].getAttribute("value") <= 5) ||
					    (this.oNds[0].getAttribute("value") >= 7) && (this.oNds[0].getAttribute("value") <= 11))
					{
			...
			if (this.sMsgBdy != "") 
			{
				...
				//this.sMsgBdy = this.sMsgHder + this.sMsgBdy;
				//alert(this.sMsgBdy);
				msgBox(this.sMsgBdy);
			}
		}
...
//	}
//	catch(loException) {
//		this.exception(oException,"UltraCargo[showMessage]");
//		return false;
//	}
//	finally {
		this.bError = null;
...
//	}
//	catch(loException) {
//		this.exception(oException,"UltraCargo[showMessageList]");
//		return false;
//	}
//	finally {
		this.bError = null;
			...
			if (this.oNd != null) {
                ...
                {				
				    ...
				    for (var iR = 0; iR < this.oNds.length; iR++) 
				    {
					    ...
					    {
						    this.bPK = "false";
						    this.sVl = (this.oNds[iR].childNodes[iC].getAttribute("value") != null) ? unescape(this.oNds[iR].childNodes[iC].getAttribute("value")) : "";
						    this.oNds[iR].childNodes[iC].setAttribute("server_value",escape(this.sVl));
						    if (this.sPK != "") {
							    if (("|" + this.sPK + "|").indexOf("|" + this.oNds[iR].childNodes[iC].nodeName + "|") != -1) {
								    this.sXPath = "root/" + this.sFrmId + "/row/" + this.oNds[iR].childNodes[iC].nodeName + "[@value='" + escape(this.sVl) + "']";
						    ...
						    if (this.bPK == "false") 
						    {
							    this.oCol = this.init.oXD.selectSingleNode("root/" + this.sFrmId + "/row/" + this.oNds[iR].childNodes[iC].nodeName);
							    if (this.oCol == null) 
							    {
								    this.bPK = "";
							    }
						    ...
						    if (this.sHidFlds != "") 
						    {
							    if (("|" + this.sHidFlds + "|").indexOf("|" + this.oNds[iR].childNodes[iC].nodeName + "|") != -1) 
							    {
								    this.bRemoveDataRow = false;
								    this.bPK = "false";
							    }
						    ...
						    if (this.bPK != "") 
						    {
							    this.oNds[iR].childNodes[iC].setAttribute("pk",this.bPK);
							    this.oRow.appendChild(this.oNds[iR].childNodes[iC].cloneNode(true));
						    }
					    }
					    if (this.bRemoveDataRow) 
					    {
						    if (this.oDtRow != null && this.bExecuteOnClientServer == true)
						    {
...
//				}
		...
		this.generateTableContent(this.sFrmId, psXML);
		
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[saveXmlGrid]");
//	}
//	finally {
		this.oXD = null;
		this.oNd = null;
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[lock]");
//	}
//	finally {
//	}
}
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[unlock]");
//	}
//	finally {
//	}
}
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[disableControls]");
//	}
//	finally {
		this.oNds = null;
		this.oNd = null;
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[checkStringValue]");
//		return "";
//	}
//	finally {
//	}
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[getValue]");
//		return "";
//	}
//	finally {
		this.sResult = null;
...
//	}
//	catch(oException) {
//		this.exception(oException,"UltraCargo[updateStruct]");
//	}
//	finally {
		this.oNds = null;
//  }
...
//	try {
		...
		if (this.exception.sBody != "") {
			...
			//alert(this.exception.sBody);
			msgBox(this.exception.sBody);
		}
//	}
//	catch(loException) {
//		alert("Exception: [" + poException.number + "] " + loException.description);
//	}
//	finally {
...
/*
		if (opener != null) {
			this.msgBox.sHost = self.opener.parent.top.location.toString();
			this.msgBox.sHost = this.msgBox.sHost.substring(0,this.msgBox.sHost.lastIndexOf("/"));
		}
		else {
			this.msgBox.sHost = parent.top.location.toString();
			this.msgBox.asHost = this.msgBox.sHost.split("//");
			this.msgBox.sHost = (this.msgBox.asHost[0] + "//");
			this.msgBox.asHost = this.msgBox.asHost[1].split("/");
			for (var iH = 0; iH < 2; iH++) {
				this.msgBox.sHost += (this.msgBox.asHost[iH] + "/");
			}
			this.msgBox.sHost = this.msgBox.sHost.substring(0,(this.msgBox.sHost.length - 1));
		}
*/
		...
		this.msgBox.sResp = window.showModalDialog(this.msgBox.sHost + "/includes/mensagem.asp",this.msgBox.asFeatures,"dialogHeight:" + this.msgBox.iHeight + "px;dialogWidth:350px;center:yes;help:no;resizable:no;status:no;scroll:no");
...
//	}
//	catch(loException) {
//			("Exception: [" + poException.number + "] " + loException.description);
//	}
//	finally {
//	}
}
...
{
		//Desabilitado por padrao, caso queira o arquivo de log, coloque um if (true)
		//Configurar o browser para aceitar 
		//internet options/security/local intraweb / custom level / initialize and script ActiveX not marked as safe : enabled
//		if (true)
		if (false)
		{
		try
		...
		catch(excpt) //caso o browser nao estiver configurado.
			{
